...
async def node_with_params(request):
    ...
    async for (sims, wallets, bt) in setup_simulators_and_wallets(1, 0, test_constants_modified, **params):
        yield sims[0]
...
async def setup_four_nodes(db_version):
    async for _ in setup_simulators_and_wallets(4, 0, test_constants_modified, db_version=db_version):
        yield _


@pytest_asyncio.fixture(scope="function")
async def two_nodes_sim_and_wallets():
    async for _ in setup_simulators_and_wallets(2, 0, test_constants_modified):
        yield _
...
async def one_wallet_and_one_simulator_services():
    async for _ in setup_simulators_and_wallets_service(1, 1, test_constants_modified):
        yield _


@pytest_asyncio.fixture(scope="function")
async def wallet_node_100_pk():
    async for _ in setup_simulators_and_wallets(1, 1, test_constants_modified, initial_num_public_keys=100):
        yield _
...
async def simulator_and_wallet() -> AsyncIterator[
    ...
    async for _ in setup_simulators_and_wallets(1, 1, test_constants_modified):
        yield _
...
async def two_wallet_nodes(request):
    ...
    if request and request.param_index > 0:
        params = request.param
    async for _ in setup_simulators_and_wallets(1, 2, test_constants_modified, **params):
        yield _
...
async def two_wallet_nodes_services() -> AsyncIterator[
    ...
    async for _ in setup_simulators_and_wallets_service(1, 2, test_constants_modified):
        yield _
...
async def two_wallet_nodes_custom_spam_filtering(
    ...
    async for _ in setup_simulators_and_wallets(
        1, 2, test_constants_modified, spam_filter_after_n_txs, xch_spam_amount
    ):
        yield _
...
async def three_sim_two_wallets():
    async for _ in setup_simulators_and_wallets(3, 2, test_constants_modified):
        yield _


@pytest_asyncio.fixture(scope="function")
async def setup_two_nodes_and_wallet():
    async for _ in setup_simulators_and_wallets(2, 1, test_constants_modified, db_version=2):
        yield _
...
async def setup_two_nodes_and_wallet_fast_retry():
    async for _ in setup_simulators_and_wallets(
        1, 1, test_constants_modified, config_overrides={"wallet.tx_resend_timeout_secs": 1}, db_version=2
    ):
        yield _
...
async def three_wallet_nodes():
    async for _ in setup_simulators_and_wallets(1, 3, test_constants_modified):
        yield _


@pytest_asyncio.fixture(scope="function")
async def wallet_two_node_simulator():
    async for _ in setup_simulators_and_wallets(2, 1, test_constants_modified):
        yield _
...
async def wallet_nodes_perf():
    async_gen = setup_simulators_and_wallets(
        1, 1, test_constants_modified, config_overrides={"MEMPOOL_BLOCK_BUFFER": 1, "MAX_BLOCK_COST_CLVM": 11000000000}
    )
    nodes, wallets, bt = await async_gen.__anext__()
    full_node_1 = nodes[0]
    server_1 = full_node_1.full_node.server
...
async def wallet_nodes_mainnet(db_version):
    async_gen = setup_simulators_and_wallets(2, 1, test_constants_modified, db_version=db_version)
    nodes, wallets, bt = await async_gen.__anext__()
    full_node_1 = nodes[0]
    full_node_2 = nodes[1]
    server_1 = full_node_1.full_node.server
...
async def one_node() -> AsyncIterator[Tuple[List[Service], List[FullNodeSimulator], BlockTools]]:
    async for _ in setup_simulators_and_wallets_service(1, 0, test_constants_modified):
        yield _


@pytest_asyncio.fixture(scope="function")
async def one_node_one_block() -> AsyncIterator[Tuple[Union[FullNodeAPI, FullNodeSimulator], ChiaServer, BlockTools]]:
    async_gen = setup_simulators_and_wallets(1, 0, test_constants_modified)
...
async def two_nodes_one_block():
    async_gen = setup_simulators_and_wallets(2, 0, test_constants_modified)
    nodes, _, bt = await async_gen.__anext__()
    full_node_1 = nodes[0]
    full_node_2 = nodes[1]
    server_1 = full_node_1.full_node.server
...
async def farmer_one_harvester_simulator_wallet(
    ...
    async for sim_and_wallet in setup_simulators_and_wallets_service(1, 1, test_constants_modified):
...
async def simulation(bt):
    async for _ in setup_full_system(bt.constants, bt, db_version=1):
        yield _
